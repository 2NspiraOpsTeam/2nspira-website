...
export default function BillingPaymentCenter({ methods, invoices, canManage, managedAchFeeCents, cardFeeBasisPoints }: { methods: Method[]; invoices: Invoice[]; canManage: boolean; managedAchFeeCents: number; cardFeeBasisPoints: number }) {
  ...
  const [policy, setPolicy] = useState({ managedAchFeeCents, cardFeeBasisPoints });
  useEffect(() => { void fetch("/api/portal/payments/policy").then((response) => response.ok ? response.json() : null).then((value) => { if (value) setPolicy(value); }); }, []);
  ...
  return <div className="portal-payment-center">
    ...
    <section className="portal-payment-choices"><h2>Open invoices</h2>{invoices.length === 0 ? <p>No open invoices.</p> : invoices.map((invoice) => <InvoicePayment key={invoice.id} invoice={invoice} methods={methods} canManage={canManage} managedAchFeeCents={policy.managedAchFeeCents} cardFeeBasisPoints={policy.cardFeeBasisPoints} />)}</section>
  </div>;
}
...
function InvoicePayment({ invoice, methods, canManage, managedAchFeeCents, cardFeeBasisPoints }: { invoice: Invoice; methods: Method[]; canManage: boolean; managedAchFeeCents: number; cardFeeBasisPoints: number }) {
  ...
  const cardFee = Math.round(invoice.balance * 100 * cardFeeBasisPoints / 10_000); const managedFee = managedAchFeeCents;
  const feeCents = choice === "managed_ach" ? managedFee : choice === "card" ? cardFee : 0;
  async function pay() {
  ...
  return <article className="portal-invoice-payment"><header><strong>{invoice.invoiceNumber}</strong><span>${invoice.balance.toFixed(2)} · due {invoice.dueDate}</span></header><div className="portal-choice-grid">
    ...
    <label><input type="radio" checked={choice === "managed_ach"} onChange={() => setChoice("managed_ach")} /> Managed ACH <small>${(managedFee / 100).toFixed(2)} disclosed convenience fee · total ${(invoice.balance + managedFee / 100).toFixed(2)}</small></label>
    <label><input type="radio" checked={choice === "card"} onChange={() => setChoice("card")} /> Credit Card <small>${(cardFee / 100).toFixed(2)} configured processing fee when selected</small></label>
